...
- Also run a compile check for the Skia framebuffer backend:
  - `SCENIC_LOCAL_TARGET=skia-fb mix compile` (set `SKIA_CFLAGS`/`SKIA_LDFLAGS` if they are not already available in the environment).
  - Record the result in your notes/tests, even if dependencies are missing.
